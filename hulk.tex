...
\MC{} defined a normed $\mathbb{Z}$-module (on $R$) as a
$\mathbb{Z}$-module equipped with a norm with values in a numeric domain $R$,
and a numeric domain as a $\mathbb{Z}$-module on itself, and this apparent
cyclicity which was encodable by hand is not supported either by \HB{}.
...
and enable the porting of \MC{}-analysis. There are also hierarchies of
morphisms and subobjects in \MC{}, while we could have also ported the former
the latter follows a different design pattern that is not yet
supported by \HB{}. Additionally \HB{} does not yet have a built-in check for
forgetful inheritance~\cite{affeldt:hal-02463336}.
